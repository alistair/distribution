...
func (ms *manifestStore) EnumerateReferrer(ctx context.Context, dgst digest.Digest, artifactType string, ingester func(v1.Descriptor) error) error {
	...
	err := handler.references.Enumerate(ctx, dgst, artifactType, func(d digest.Digest) error {
		desc, err := ms.blobStore.statter.Stat(ctx, d)
		if err != nil {
